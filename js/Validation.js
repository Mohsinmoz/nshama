...
function ValidateFileCv(lbl, file) {
    var val = file.val().toLowerCase();
    //(.*?)\.(jpg|jpeg|txt|png|docx|gif|doc|pdf|xml|bmp|ppt|pptx|vsd|xls|xlt|xlsx)$
    //(.*?)\.(jpg|jpeg|txt|docx|doc|pdf|ppt|pptx)$
    var regex = new RegExp("(.*?)\.(rtf|docx|doc|pdf|ppt|pptx)$");
    if ((regex.test(val))) {
        lbl.removeClass(CssErrorClassName);
        // $(".Browse").removeClass(CssErrorClassName);
        return true;
    ...
    function checkfilesize(lbl) {

        var size = document.getElementById("<%=fuCv.ClientID%>").files[0].size;
        if (size > '10485760') {
    ...
    function ValidateMobileonly( txt) {
        var re = /^\+?[\d\s]{6,20}$/;
        if (txt.val().length > 0) {
            if (txt.val().trim().match(re)) {
    ...
    function ValidateName(lbl, txt, maxLength) {
        var re = /^(([a-zA-Z ]+)$|([ء-ي ]+)$)/;
